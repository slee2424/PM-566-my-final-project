...
- Based on th analysis above, I built a prediction model. The final training model concluded with around 80% Accuracy, which indicates good prediction ability of the model. Along with the good accuracy value, 66.7% Sensitivity, 88.3% Specificity, 76.7% Positive Prediction Value, and 82.0% Negative Prediction Value was obtained from the model.
...
```{r}
final_model <- glm(outcome ~ dpf + lip + glu + ins + bmi + ins*dpf, family=binomial, data=db_train) 
```
