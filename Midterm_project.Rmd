...
- Created age groups to compare proportion of the diabetes by age groups. 
...
```{r}
mfp::mfp(outcome ~ fp(dpf), 
         family = poisson,
         data = db)

model1<- glm(outcome ~ dpf, 
         family = poisson,
         data = db)
...
```{r}
mfp::mfp(outcome ~ fp(lip), family = poisson, data = db)
glm(outcome ~ lip, family = poisson, data = db) %>% summary()
```

```{r}
mfp::mfp(outcome ~ fp(glu), family = poisson, data = db)
glm(outcome ~ I(glu^-2), family = poisson, data = db) %>% summary()
```

```{r}
mfp::mfp(outcome ~ fp(bp), family = poisson, data = db)
glm(outcome ~ bp, family = poisson, data = db) %>% summary()
```

```{r}
mfp::mfp(outcome ~ fp(st), family = poisson, data = db)
glm(outcome ~ st, family = poisson, data = db) %>% summary()
```

```{r}
mfp::mfp(outcome ~ fp(ins), family = poisson, data = db)
glm(outcome ~ ins, family = poisson, data = db) %>% summary()
```

```{r}
mfp::mfp(outcome ~ fp(bmi), family = poisson, data = db)
glm(outcome ~ I(bmi^-2), family = poisson, data = db) %>% summary()
```

```{r}
mfp::mfp(outcome ~ fp(age), family = poisson, data = db)
glm(outcome ~ age, family = poisson, data = db) %>% summary()
```


subset_db_prelim <-
  glmulti::glmulti(outcome ~ dpf + lip + I(glu^-2) + bp + st + ins + I(bmi^-2) + age,
                   data = db,
                   level=2, family = binomial, crit="aicc", confsetsize=128)
```{r}
model2 <- lm(outcome ~ dpf + lip + glu + bp + st + ins + bmi + age, data=db)

mfp::mfp(outcome ~ fp(dpf)+ lip + I(glu^-2) + bp + st + ins + I(bmi^-2) + age, 
         family = poisson,
         data = db)

glm(outcome ~ dpf + lip + I(glu^-2) + bp + st + ins + I(bmi^-2) + age, 
         family = poisson,
         data = db) %>% summary()
